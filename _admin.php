...
class GalleryInsertbehaviorContent
{
    ...
    public static function coreAfterPostContentFormat ($arr)
    {
		...
		$arr['excerpt_xhtml'] = preg_replace_callback($galerie_reg, [__CLASS__, 'ParseGalleryInsert'], $arr['excerpt_xhtml']);
        //$arr['content_xhtml'] = preg_replace_callback($galerie_reg, ['self', 'ParseGalleryInsert'], $arr['content_xhtml']);
		$arr['content_xhtml'] = preg_replace_callback($galerie_reg, [__CLASS__, 'ParseGalleryInsert'], $arr['content_xhtml']);
    }
	...
	protected static function CreateRepGalleryInsert($dir_name, $linkto, $thumbsize)
{
    ...
    if (!is_dir($my_media->root . '/' . $dir_name)) {
        return "<!-- Répertoire \"$dir_name\" inexistant -->";
    }

    // Accès au contenu du répertoire
    $my_media->chdir($dir_name);
    $my_media->getDir();
    $f = $my_media->dir;

    // Parcours des fichiers et insertion
    foreach ($f['files'] as $v) {
        if (isset($v->type) && strpos($v->type, 'image/') === 0) {
    ...
    protected static function CreateImgUrlGalleryInsert($listimages, $linkto, $thumbsize)
{
    ...
    // Liste d’images séparées par ";"
    $listimages = explode(';', $listimages);
    $dir_name = trim(array_shift($listimages), "/");
    ...
    // Pour chaque image explicitement demandée
    foreach ($listimages as $img_name) {
        foreach ($f['files'] as $file_obj) {
            // Correspondance par nom de fichier (basename)
            if ($file_obj->basename === $img_name) {
                $s .= self::InsertImage($file_obj, $linkto, $thumbsize);
                break; // On passe à l'image suivante
            }
        }
    }
