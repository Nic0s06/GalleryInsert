...
$core->addBehavior('publicHeadContent',array('GalleryInsertbehaviorContent','publicHeadContent'));
...
//if ($settings->galleryinsert_enabled && $settings->privatepicture_enabled) {
	$core->addBehavior('publicBeforeContentFilter',array('GalleryInsertbehaviorContent','publicBeforeContentFilter'));
//}
...
class GalleryInsertbehaviorContent {
	...
	public static function publicHeadContent($core)
	{
		...
		if ($settings->divbox_enabled) {
			echo '<script type="text/javascript" src="'.$url.'/divbox/divbox.js"></script>'."\n";
			echo '<link type="text/css" rel="stylesheet" href="'.$url.'/divbox/css/divbox.css" />'."\n";
		}

		// Inclusion de jcarousel
		if ($settings->carousel_enabled) {
			echo '<script type="text/javascript" src="'.$url.'/tosrus/jquery.tosrus.min.all.js"></script>'."\n";
		...
		if ($settings->galleria_enabled) {
			echo '<script type="text/javascript" src="'.$url.'/galleria.1.5.7/galleria-1.5.7.min.js"></script>'."\n";
		...
		if ($settings->jgallery_enabled) {
			...
			//echo '<script type="text/javascript" src="js/jquery-2.0.3.min.js"></script>'."\n";
			echo '<script type="text/javascript" src="'.$url.'/jgallery-1.6.2/js/jgallery.min.js"></script>'."\n";
			echo '<script type="text/javascript" src="'.$url.'/jgallery-1.6.2/js/touchswipe.min.js"></script>'."\n";
		}
	...
	public static function publicBeforeContentFilter ($core,$tag,$args)
	{
		...
		if ($tag == "EntryContent" || $tag == "EntryExcerpt") {
			...
			if (preg_match($reg, $args[0])) {
				...
				$args[0] = preg_replace_callback($reg, array('self','ProcessPrivatePictures'), $args[0]);
	...
	protected static function ProcessPrivatePictures($m) {
		...
		if (preg_match($reg, $chaine, $match)) {
			...
			if (isset($_SESSION['sess_blog_showprivatepictures']) && $_SESSION['sess_blog_showprivatepictures'] == $match[1]) {
				...
				<form id="'.$formname.'" method="post" action="#gallery'.self::$gallerynumber.'"><p>' . $GLOBALS['core']->formNonce() . form::hidden('showprivatepictures','0') . '</p></form>';
			} else {
				...
				' . $GLOBALS['core']->formNonce() . form::hidden('showprivatepictures','1') .
	...
	public static function publicBeforeContentFilter2 ($core,$tag,$args)
	{
		global $_ctx;
		...
		if ($tag == "EntryContent" || $tag == "EntryExcerpt") {
			...
			while (preg_match($reg, $args[0], $match)) {
				...
				if (isset($_SESSION['sess_blog_showprivatepictures']) && $_SESSION['sess_blog_showprivatepictures'] == $match[1]) {
					...
					<form name="'.$formname.'" method="post">' . $GLOBALS['core']->formNonce() . form::hidden('showprivatepictures','0') . '</form>';
				} else {
						// Suppression des images privées
